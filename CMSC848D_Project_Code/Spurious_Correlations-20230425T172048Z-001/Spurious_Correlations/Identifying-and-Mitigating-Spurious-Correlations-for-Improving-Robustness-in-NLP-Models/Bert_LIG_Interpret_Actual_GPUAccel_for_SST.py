...
def run(rank, size, inp_batch,label_batch):
	...
	for i in range(7500):
		...
		attr = lig.attribute(tok_input_batch.input_ids, target=curr_lab_batch).detach().cpu()
...
def init_process(rank, size, fn, inp_batch,label_batch, backend='gloo'):
    ...
    filename=f'Attributions/LIG/Big/SST/list_LIG_7500samples_GPU_trainidx_trainlabel_attributes_{rank}'

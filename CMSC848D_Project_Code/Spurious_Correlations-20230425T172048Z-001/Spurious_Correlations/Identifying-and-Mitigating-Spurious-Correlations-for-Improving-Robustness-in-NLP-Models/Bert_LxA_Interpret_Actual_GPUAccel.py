...
def run(rank, size, inp_batch,label_batch):
	...
	for i in range(7500):
		...
		count+=1
		# print(model)
		lga = LGA(fun_predict_captum,model.module.distilbert.embeddings)
		attr = lga.attribute(tok_input_batch.input_ids, target=curr_lab_batch).detach().cpu()
...
def init_process(rank, size, fn, inp_batch,label_batch, backend='gloo'):
    ...
    os.environ['MASTER_PORT'] = '29590' # Figure out later
    ...
    filename=f'Attributions/LxA/Big/Yelp/list_LxA_7500samples_GPU_trainidx_trainlabel_attributes_{rank}'
...
#	print(split_3)
	...
	batch_chunks = (df_chosen['text'][:split_1], df_chosen['text'][split_1:split_2], df_chosen['text'][split_2:split_3])

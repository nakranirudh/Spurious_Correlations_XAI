...
def get_yelp():
    ...
    df = pd.DataFrame(yelp_dataset['test'])
    
    return df
    
def get_sst2():
    sst2_dataset = load_dataset(
            "glue",
            'sst2',# HG HARD SETTING THIS FOR NOW
            #cache_dir=model_args.cache_dir,
            # use_auth_token=True if model_args.use_auth_token else None,
        )
    df = pd.DataFrame(sst2_dataset['train'])
    df.rename(columns={'sentence': 'text'}, inplace=True)
    
    return df

def get_movie_rationales():
    movie_rationales_dataset = load_dataset("movie_rationales")
    df = pd.DataFrame(movie_rationales_dataset['train'])

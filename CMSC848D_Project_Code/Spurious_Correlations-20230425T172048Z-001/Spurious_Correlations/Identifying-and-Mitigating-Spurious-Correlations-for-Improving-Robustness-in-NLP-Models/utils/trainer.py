...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            if isinstance(train_dataloader, DataLoader) and isinstance(train_dataloader.sampler, DistributedSampler):
                print("Are we dealing with a valid dataloader?")
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        ...
        output, attention_weights = self.prediction_loop(eval_dataloader, description="Evaluation")
        ...
        return output.metrics, attention_weights, output
    ...
    def prediction_loop(
        ...
        batch_size = 1
        print("Batch_size is now: ", batch_size)
        ...
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            loss, logits, labels, attention_weights_ = self.prediction_step(model, inputs, prediction_loss_only)
            attention_weights.append(attention_weights_.cpu().detach())
            batch_size = inputs[list(inputs.keys())[0]].shape[0]
            if loss is not None:
                eval_losses.extend([loss] * batch_size)
            if logits is not None:
                preds = logits if preds is None else nested_concat(preds, logits)
            if labels is not None:
                label_ids = labels if label_ids is None else nested_concat(label_ids, labels)
        ...
        return PredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics), \
                torch.cat(attention_weights, dim=0)
    ...
    def prediction_step(
        ...
        inputs = self._prepare_inputs(inputs)
        with torch.no_grad():
            outputs = model(**inputs, output_attentions=True) # attention: num_layerx[bs, num_head, seq_len, seq_len]
            if has_labels:
                ...
                #pdb.set_trace()
                logits = outputs[1]
            # be careful with XLNet and TansfoormerXL
            if self.args.past_index >= 0:
        ...
        return (loss, logits, labels, torch.mean(outputs[-1][-1], dim=1, keepdim=False)[:, 0, :])
